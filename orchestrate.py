...
def get_req_csv_from_s3(monthly_dir_name):    
    ...
    local_gz_filename = 'billing_report_'+timestamp+'_'+dtdt.strptime(monthly_dir_name.split('-')[0],'%Y%m%d').strftime('%Y-%m')+'.csv.gz'
...
def index_csv(filename, dir_name, template=True):
    ...
    index_format = dtdt.strptime(dir_name.split('-')[0],'%Y%m%d').strftime('%Y.%m')
    os.environ['file_y_m'] = index_format
    ### have to change the name of the index in logstash index=>indexname
    
    status = subprocess.Popen(['curl -XGET elasticsearch:9200/_cat/indices/?v'], shell=True)
    ...
    status = subprocess.Popen(['go run /aws-elk-billing/main.go --file /aws-elk-billing/'+filename], shell=True)
    if status.wait() != 0:
...
for i in index_list:
    if i:
        index_time.append(es.search(index=i, size=1, query={"query": {"match_all": {}}})['hits']['hits'][0]['_source']['@timestamp'])
...
if index_time:
    ...
    last_ind_dir = dtdt.strptime(last_ind_dir,'%Y%m%d').strftime('%Y%m01')+'-'+(dtdt.strptime(last_ind_dir,'%Y%m%d')+relativedelta(months=1)).strftime('%Y%m01')
